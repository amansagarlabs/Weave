...
const CrowdCanvas = ({ src, rows = 7, cols = 15 }: CrowdCanvasProps) => {
  ...
  useEffect(() => {
    ...
    const resetPeep = ({ stage, peep }: { stage: any; peep: any }) => {
      ...
      let offsetB = 250;
      let scale = 1;
      if (window.innerWidth < 960) { offsetA = 200; offsetB = 150; scale = 0.5; }
      if (window.innerWidth < 480) { offsetA = 240; offsetB = 100; scale = 0.35; }
      const direction = Math.random() > 0.5 ? 1 : -1;
      const offsetY = offsetA - offsetB * gsap.parseEase("power2.in")(Math.random());
    ...
    const createPeep = ({
      ...
      const peep: Peep = {
        ...
        render: (ctx: CanvasRenderingContext2D) => {
          const scaleY = ctx.canvas.clientWidth < 480 ? 0.35 : ctx.canvas.clientWidth < 960 ? 0.5 : 1;
          ctx.save();
          ctx.translate(peep.x, peep.y);
          ctx.scale(peep.scaleX, scaleY);
          ctx.drawImage(
            peep.image,
            peep.rect[0],
            peep.rect[1],
