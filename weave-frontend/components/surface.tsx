...
export function DiscoveryCard({ name, category, price }: { name: string; category: string; price: string }) { return <Card className="group overflow-hidden p-0"><div className="flex aspect-[4/3] items-center justify-center bg-[var(--accent)] text-5xl transition-transform group-hover:scale-[1.02]" aria-hidden="true">✦</div><div className="p-5"><div className="flex items-center justify-between"><h3 className="font-black">{name}</h3><Pill>{category}</Pill></div><p className="mt-3 text-sm text-[var(--muted)]">Available for work · 18k followers</p><div className="mt-5 flex items-center justify-between"><span className="text-sm font-bold">From {price}</span><ButtonLink href="/brand/creator/sample" variant="outline">View profile</ButtonLink></div></div></Card>; }
