...
Deal Detail (status: accepted) → mark "content delivered" → ASCI Checklist shown → Invoice Generator (pre-filled from deal) → Invoice sent via UniBee checkout → Invoice Detail shows "sent" → UniBee webhook updates to "paid" → Deal History updated automatically
